...
def pokemon_list(request):
    ...
    paginator = Paginator(pokemons, 12)  # Show 12 per page
...
def pokemon_update(request, id):
    pokemon = get_object_or_404(Pokemon, id=id)
    if request.method == "POST":
        form = PokemonForm(request.POST, instance=pokemon)  # Pass the specific Pokémon instance
        if form.is_valid():
            form.save()
            return redirect("pokemon_list")
    else:
...
# Delete a Pokémon (not implemented here, just a placeholder)
def pokemon_delete(request, id):
    pokemon = get_object_or_404(Pokemon, id=id)
    if request.method == "POST":
        pokemon.delete()
        return redirect("pokemon_list")
    return render(request, "pokemon_confirm_delete.html", {"pokemon": pokemon})
